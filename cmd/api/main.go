...
// Add maxOpenConns, maxIdleConns and maxIdleTime field to hold the configuration
// settings for the connectin pool.
type config struct {
	port int
	env  string
	db   struct {
...
func main() {
	...
	// Note the default values that we're using?
...
func openDB(cfg config) (*sql.DB, error) {
	...
	// Set the maximum number of open (is-use + idle) connections in the pool. Note that
	// passing a value less than or equal to 0 will mean there is no limit.
	db.SetMaxOpenConns(cfg.db.maxOpenConns)

	// Set the maximum number of ilde connections in the pool. Again, passing a value
	// less than or equal to 0 will mean there is no limit.
	db.SetMaxIdleConns(cfg.db.maxIdleConns)

	// Use the time.ParseDuration() function to convert to idle timeout duration string
